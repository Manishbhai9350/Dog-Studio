...
const Project = ({ headline = "", title = "", description = "" }) => {
  ...
  useGSAP(() => {
    ...
    if(hovered) {
      ...
      DescLines.forEach((line,i) => {
        gsap.to(line,{
          delay:.2 + .1/2 * (DescLines.length - (i + 1)),
          y:-30,
          opacity:0,
          stagger:.06,
        })
  ...
  return (
    <div className="project w-full max-lg:!my-14">
      ...
      <div className="desktop max-lg:hidden w-full cursor-pointer group">
        <h1 onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="text-7xl heebo-v font-[100] opacity-40 group-hover:opacity-100 !transition-opacity !duration-200 lg:!py-7">
          {title}
        </h1>
        <div
          ref={container}
        ...
        >
          ...
          {lines.map((line, i) => (
            <p key={i} className="desktop-description-line opacity-0 relative text-xl heebo-light w-full text-right">
              {line}
            </p>
          ))}
        </div>
